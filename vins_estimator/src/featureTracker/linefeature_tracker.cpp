...
//  TODO: why改成鱼眼模型去畸变-----此处修改的代码目测没什么用-----------------------------------------------
...
void LineFeatureTracker::readImage(const cv::Mat &_img)
{
    ...
    if(curframe_->vecLine.size() > 0)
    {
        ...
        stringstream ss;
        ss <<"/home/why/Desktop/musti_feature_Fusion/PEVINS/VINS_FUSION_PL_New/src/VINS-Fusion/result/line/" <<frame_cnt<<".jpg";
                          ...
                          std::vector<DMatch> lsd_matches)
{
    ...
    for (int k = 0; k < good_matches.size(); ++k) {
        ...
        // why
        // double len = std::min(1.0, 1.0 * linetrack_cnt[k] / 20);
        ...
        // cv::line(img1, startPoint, endPoint, cv::Scalar(255*(1-len),0,255*len),2 ,8); // why
    ...
    string Img_Name = "/home/why/Desktop/musti_feature_Fusion/PEVINS/VINS_FUSION_PL_New/src/VINS-Fusion/result/Image-line/" +to_string(k++)+".png";
    imwrite(Img_Name,img1);
...
// map<int, vector<pair<int, Vector4d>>> LineFeatureTracker::trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1)
{
    ...
    /**Void remap(InputArray src,OutputArray dst,InputArray map1,InputArray map2,
      ...
      borderValue：表示插值数值
     */
    cv::remap(_img, img, undist_map1_, undist_map2_, CV_INTER_LINEAR); // 此处如果使用mei格式，则会放大、、、、
...
//        ROS_INFO("lbd_macht costs: %fms", t_match.toc());
    ...
    // why : 将 linefeature_tracker_node.cpp里面的线特征信息整合到linefeature_tracker.cpp中
